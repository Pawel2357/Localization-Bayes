...
class komorka:
    ...
    def __init__(self, P_old , P_new, position):
        ...
        self.transition = {} # dictionary with transition probabilities of going out of the cell
    ...
    def calculate_measurement_probab(self):
        ...
        self.transition['D'] = 0.25
        self.transition['U'] = 0.25
        self.transition['L'] = 0.25
        self.transition['R'] = 0.25
    ...
    #based on information where robot move(north or south) I calculate transition probabilities to this cell
    # INPUT : direction - how robot moved 'L', 'R', 'U', 'D'
    # OUTPUT : updated self.transition based on direction of last move
    def calculate_transition_probab(self, direction):
        
        p = False # no walls initialization (no walls in the same direction as robot move)
        ...
        for i in self.transition:
            if i==direction:
                if self.walls[i]=='N':
                    ...
                    self.transition[i] = 0.0
                    p = True # walls 
                    break
        # if direction is the same as wall than I update transition probabilites to be the same in all directions
        if p:
            ...
            if num_walls == 0:
                for i in self.walls:
                    self.transition[i] = 1.0/4.0
                    
            if num_walls == 1:
                for i in self.walls:
                    if self.walls[i] == 'N':
                        self.transition[i] = 1.0/3.0
                    else:
                        self.transition[i] = 0.0
    ...
    # calculate posteriori probability of finding robot in this position, update transition probabilities
    # INPUT : measurement - list of walls that robot observe, states - list of all states, P_measurement - probability
    # of getting measurement, direction - direction of last move
    # OUTPUT : calculate P_new using Bayes rule, updated transition
    def update_probabilities(self, measurement, states, P_measurement):
        ...
        for neighbour in self.neighbours:
            if neighbour.position[0] == self.position[0]:
                if neighbour.position[1] == self.position[1] - 1:   
                    P_position += neighbour.transition['U'] * neighbour.P_old
                else:
                    P_position += neighbour.transition['D'] * neighbour.P_old
                
            if neighbour.position[1] == self.position[1]:
                if neighbour.position[0] == self.position[0] - 1:
                    P_position += neighbour.transition['R'] * neighbour.P_old
                else:
                    P_position += neighbour.transition['L'] * neighbour.P_old
...
class grid_world:
    ...
    def __init__(self, position, states):
        ...
        self.direction = 'R' # direction of last move 'L', 'D', etc.
    ...
    def perform_measurement(self):
        ...
        for i in range(len(self.states)):
            ...
            self.states[i].calculate_transition_probab(self.direction)
    ...
    def p_measurement(self):
        ...
        for i in self.states:
            ...
            for neighbour in i.neighbours:
                if neighbour.position[0] == i.position[0]:
                    if neighbour.position[1] == i.position[1] - 1:   
                        P_position += neighbour.transition['U'] * neighbour.P_old
                    else:
                        P_position += neighbour.transition['D'] * neighbour.P_old
                    
                if neighbour.position[1] == i.position[1]:
                    if neighbour.position[0] == i.position[0] - 1:
                        P_position += neighbour.transition['R'] * neighbour.P_old
                    else:
                        P_position += neighbour.transition['L'] * neighbour.P_old
    ...
    def robot_localization(self, num_steps):
        #self.direction = 'U' what should be here?
